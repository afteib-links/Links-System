(() => {
  ...
  const featureSummary = {
    master_data_preparation: '旧原本Excelを編集用の形式へ変換します。通常の変換では業務データをDBへ登録しません。基盤マスター初期値の選択復旧は、この画面の別操作です。',
    db_import: '編集済みExcelをDBへ登録・更新する工程です。新規、更新、変更なし、エラー、依存先エラー、競合を先に確認し、正常行だけを登録します。',
    db_export: '現行DBの企業・請求先・車両・パートナー・案件・料金を編集用Excelに出力します。手入力したレコードも対象です。',
    master_data_export: 'システムに同梱された基盤マスター初期値Excelを取得します。現在の企業・パートナーなどの登録値を取り出すDB出力とは内容が異なります。',
    test_data: '検証専用環境・検証専用DBでサンプル確認から日報、月次承認、精算、帳票まで段階的に生成します。メニューが表示されても通常環境では生成できません。',
    calculation_rules: '日次計算、月次集約、控除、税・丸めなどの処理を版管理します。金額データは契約単価、この画面は計算の処理条件を管理するために分かれています。',
    menu_access_settings: '役割と機能のチェック表でメニュー利用を設定します。ユーザー管理で人に役割を付け、この画面で役割に機能を割り当てます。',
