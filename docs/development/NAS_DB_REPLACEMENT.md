...
- 移行先の既存DBを置換してよいこと。置換前に移行先DBの復旧用ダンプを自動作成します。
...
この処理は、(1) 移行先の復旧用DBダンプを作成、(2) アプリ停止、(3) `links_system` を削除・再作成、(4) 復元、(5) セッション削除、(6) アプリ起動、(7) `/api/health` と件数照合、の順で実行します。単なる上書きインポートではないため、移行先にだけあったレコードは残りません。全利用者は再ログインが必要です。
