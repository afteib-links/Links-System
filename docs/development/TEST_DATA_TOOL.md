...
実装済み: 管理者向け取込、名称の維持/仮想化、コード列割当、件数・期間・勤務割合、サンプル、採否・コメント保存、版承認、匿名JSON/CSV、完成Excelから登録済みのマスターを日報サンプル設定へ連携、承認済み設定から専用検証DBへの日報生成、3サイクルの日報提出と月次承認生成、承認済み月次データからの先払・請求・支払生成。

未実装: 料金・契約期間列の取込、コードの関連付き仮想化、任意勤務帯、契約/料金改定、帳票PDF・通常支払の入出金実績・銀行CSV・正式FB・分析、生成DB移送・復元。
