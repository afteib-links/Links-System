...
Windows PowerShell・NAS SSHどちらも、リポジトリ直下から次を実行する。

```sh
docker compose --env-file tmp/test-tool.env -f docker-compose.test-data.yml up --build -d
```
...
APIテストは管理テーブルのテストダブルを使用し、既存DBには接続しない。実DBマイグレーションとDocker/NAS実機確認は別途必要。
...
- Docker起動・実DBへのmigration適用・NASでの動作は未検証。実行シェルではDocker CLIがPATH上に存在せず、既存環境は変更していない。
