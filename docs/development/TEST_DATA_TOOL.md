...
実装済み: 管理者向け取込、名称の維持/仮想化、コード列割当、件数・期間・勤務割合、サンプル、採否・コメント保存、版承認、匿名JSON/CSV、完成Excelから登録済みのマスターを日報サンプル設定へ連携、承認済み設定から専用検証DBへの日報生成、3サイクルの日報提出と月次承認生成。

未実装: 料金・契約期間列の取込、コードの関連付き仮想化、任意勤務帯、契約/料金改定、精算/先払/分析、PDF/銀行CSV、生成DB移送・復元。
