...
| 項目 | 決定内容 |
|------|----------|
| NAS CPU | **Intel（x86_64）**。Docker公式イメージ（`node` / `mariadb` の amd64）を使用する |
| ADM | Docker Engine（Compose v2）が動く現行ADMを前提。具体バージョンは配備時に記録する |
| アクセス方法 | **`http://<NASのIP>:8080`**。社内LANからのHTTPアクセス。ドメイン/HTTPSはMVP対象外 |
| 認証 | **簡易ID / パスワード**。ロールは最低でも「事務担当」「管理者」の2種。無認証公開はしない |
| 帳票 | **ブラウザ印刷（印刷用CSS）を優先**。A4レイアウトで `window.print()` 相当。サーバPDF生成は後続フェーズ |
...
* 請求書・支払明細書は画面プレビュー + **印刷用CSS（A4）**
* ブラウザの印刷ダイアログからPDF保存も可能なため、当面はサーバPDFライブラリを入れない
* 将来PDF必須になった場合も、スナップショット済みデータを流用できる構造にしておく
...
* 実行基盤は Docker Compose を標準とする（Intel / x86_64）  
* 公開URLは `http://<NASのIP>:8080`  
* 認証は簡易ID/パスワード（事務担当・管理者）  
* 帳票はブラウザ印刷CSS優先（PDFサーバ生成は後続）  
