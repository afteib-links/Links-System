...
| 対象 | コマンド | 動作 |
|------|----------|------|
| Windows手動操作（AI不要） | リポジトリ直下の `Docker更新.cmd` をダブルクリック | PowerShell更新ツールを起動し、結果を画面に残す |
...
ローカル作業ツリーとローカルDBをQNAPへ反映する繰り返し作業は、リポジトリ直下の `本番移行.cmd` を入口とする。システム／DBの事前バックアップ、DBの既存データ保持更新／完全差し替え、添付・PDFファイルの更新有無を実行時に個別選択する。詳細と初回準備は [`計画/27_本番移行ツール.md`](計画/27_本番移行ツール.md) を正とする。

DBの「更新」は既存行を削除せず、起動時マイグレーションだけを適用する。「差し替え」はローカルDBダンプを用いて本番DBを再作成する破壊的処理であり、選択した場合だけ実行する。どちらも `/api/health` の `db: "up"` を成功条件とし、3回連続失敗後は明示的な失敗回数リセットまで停止する。
