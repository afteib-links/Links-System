...
> **2026-09-09更新**: 本節の旧マトリクス記述は既存データ互換の説明として残す。新規編集の正は `計画/26_料金登録_項目種別行モデル設計.md` の `fee_items_v2`（料金カード＋項目種別行）である。

### 9.0 新料金カードと改定系列

- カードは上から先勝ち。曜日は月〜金、土、日、祝、案件休日（休）、ALLを持ち、ALLは排他とする。
- 行は料金項目名、項目種別、請求額、支払額、利益率（導出・非保存）、請求詳細名、支払詳細名を持つ。詳細名は保存のみとし、帳票接続は別作業とする。
- 曜日ボタン4区分と請求額・支払額の文字色はシステム設定で変更できる。既定の請求額文字は濃い青、支払額文字は濃い橙とし、セル背景には適用しない。料金セルは `Ctrl`＋カーソルキーで移動でき、保存後も編集画面を維持する。
- 条件式および請求・支払式は管理者限定の安全な式として保存し、同一項目種別は上から最初に一致した行を採用する。未提供変数を含む行は下書きで計算しない。
- `price_series` が安定した料金系列を表し、`price_sets` は系列内の改定版を表す。コードは `FEE-{企業ID5桁}-{連番4桁}-R{版3桁}`。
- 基本案件から個別案件へのコピーは新系列を作り系譜を保持する。基本改定の選択反映は旧基本・新基本・個別最新版を3点比較し、競合解決後に個別側の新しい改定版を作る。
- 日報は計算時の `applied_price_set_id` と計算明細を保持する。確認・承認・請求・支払済みの値は後日の料金改定や過去版修正で変更しない。
- 編集画面はデスクトップ3カード列のExcel型入力表を正とする。基本情報は上部に適用開始、適用終了、企業、名称の順で置き、備考は最下部に置く。条件式と請求・支払式は各料金行の末尾から必要時だけ展開する。1100px以下では1列の横長カードへ切り替え、行追加・削除を隠して文字と金額の表示幅を優先する。カード右上の操作はツールチップ付きアイコンとし、コピーを黄色、削除を赤色で区別する。
...
- 金額表示は通常金額を整数円、単価を既存値に小数がある場合だけ小数付きとする。料金入力セルでは`￥`を付けない。入力・保存・計算の整数円ルールは変更しない。
