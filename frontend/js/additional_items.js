(() => {
  ...
  const ui={
    ...
    async editItem(item){
      ...
      const updateScope=()=>{const daily=form.elements.item_scope.value==='day';form.elements.work_date.disabled=!daily;form.elements.work_date.required=daily;form.elements.work_date.closest('label').hidden=!daily;document.getElementById('extra-save').disabled=true;};
      form.elements.item_scope.onchange=updateScope;updateScope();
      ...
      document.getElementById('extra-calc').onclick=async()=>{
        try {
          ...
          if(v.item_scope==='day'&&!v.work_date)throw new Error('日別登録の勤務日を入力してください');
