  ...
  const LinksCashManagement = {
    ...
    async show(message = '') {
      ...
      const rows = schedules.map((s) => {
        ...
        return `<tr><td>${this.ctx.escapeHtml(s.cycle_code === 'end' ? '末日回' : `${Number(s.cycle_code)}日回`)}</td><td>${s.direction === 'outgoing' ? '出金' : '入金'}</td><td>${this.ctx.escapeHtml(String(s.scheduled_date).slice(0,10))}</td><td>${this.ctx.escapeHtml(s.counterparty_name)}</td><td>${this.ctx.escapeHtml(s.title)}</td><td class="num">${this.kit.money(s.amount)}</td><td class="num">${actual ? `${this.kit.money(actual)}${difference ? `（差額 ${this.kit.money(difference)}）` : ''}` : '-'}</td><td>${this.kit.statusBadge(s.status,this.status(s.status))}</td><td>${actions}</td></tr>`;
      }).join('');
    ...
    editor(options) {
      ...
      editor.innerHTML = `<section class="panel"><h2>手動予定</h2><form id="cash-form"><section class="form-section-card"><h3>予定内容</h3><div class="form-grid form-grid-compact"><div><label>管理回</label><select name="cash_cycle_id">${options}</select></div><div><label>区分</label><select name="direction"><option value="outgoing">出金</option><option value="incoming">入金</option></select></div><div><label>相手先</label><input name="counterparty_name" required></div><div><label>件名</label><input name="title" required></div><div><label>金額</label><input name="amount" type="number" min="1" required></div><div><label>個別予定日</label><input name="scheduled_date" type="date"></div><div class="full"><label>日付変更理由</label><input name="override_reason"></div></div></section><div class="btn-row form-actions-sticky"><button class="btn">登録</button></div></form></section>`;
    ...
    editSchedule(id, date, options) {
      const editor = document.getElementById('cash-editor');
      editor.innerHTML = `<section class="panel"><h2>予定変更</h2><form id="cash-edit-form"><section class="form-section-card"><h3>予定日・管理回</h3><div class="form-grid form-grid-compact"><div><label>管理回</label><select name="cash_cycle_id">${options}</select></div><div><label>個別予定日</label><input name="scheduled_date" type="date" value="${date}" required></div><div class="full"><label>変更理由</label><input name="override_reason" required></div></div></section><div class="btn-row form-actions-sticky"><button class="btn">変更を保存</button></div></form></section>`;
