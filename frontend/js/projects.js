(() => {
  const LinksProjects = {
    async open(ctx, options = {}) {
      ...
      const partners = await this.ctx.api('/api/lookups/partners');
    ...
    tabsHtml(active) {
      return `
        <div class="toolbar">
          <button type="button" class="btn ${active === 'projects' ? '' : 'btn-ghost'}" id="tab-projects">個別案件</button>
          <button type="button" class="btn ${active === 'base' ? '' : 'btn-ghost'}" id="tab-base">基本案件</button>
        </div>`;
    },
    ...
    bindTabs() {
      ...
      document.getElementById('tab-base')?.addEventListener('click', () => {
        this.tab = 'base';
        this.showBaseList();
      });
    },
    ...
    async showBaseList(message = '') {
      ...
      if (!res.ok || !data?.ok) {
        this.ctx.app.innerHTML = this.kit.shell('案件マスタ', `<section class="panel"><p class="error">${this.ctx.escapeHtml(data?.message || '取得失敗')}</p></section>`);
        this.kit.bindShell();
        return;
      }
      const rows = (data.base_projects || [])
        .map(
          ...
          <tr>
            ...
            <td>${this.ctx.escapeHtml(this.kit.codeLabel(this.codes.work_time_type, b.work_time_type))}</td>
            <td>
        ...
        `<section class="panel">
          ...
          <div class="table-wrap">
            <table class="data-table">
              <thead><tr><th>No</th><th>企業</th><th>テンプレ名</th><th>担当</th><th>時間種別</th><th>操作</th></tr></thead>
      ...
      document.getElementById('new-base')?.addEventListener('click', () => this.showBaseDetail(null));
      document.querySelectorAll('[data-edit-base]').forEach((btn) =>
        btn.addEventListener('click', () => this.showBaseDetail(Number(btn.getAttribute('data-edit-base'))))
      );
    ...
    async showBaseDetail(id) {
      ...
      if (id) {
        ...
        if (!res.ok || !data?.ok) {
          this.ctx.app.innerHTML = this.kit.shell('基本案件', `<section class="panel"><p class="error">${this.ctx.escapeHtml(data?.message || '取得失敗')}</p></section>`);
          this.kit.bindShell();
          return;
        }
    ...
    async showProjectList(message = '') {
      ...
      document.getElementById('new-project')?.addEventListener('click', () => this.showProjectDetail(null));
      document.querySelectorAll('[data-edit]').forEach((btn) =>
        btn.addEventListener('click', () => this.showProjectDetail(Number(btn.getAttribute('data-edit'))))
      );
    ...
    async showProjectDetail(id) {
      ...
      if (id) {
        ...
        if (!res.ok || !data?.ok) {
          this.ctx.app.innerHTML = this.kit.shell('案件詳細', `<section class="panel"><p class="error">${this.ctx.escapeHtml(data?.message || '取得失敗')}</p></section>`);
          this.kit.bindShell();
          return;
        }
            ...
            <div class="form-grid">
              ...
              <div><label>支払区分</label>
                <select name="payment_type">
                  <option value="normal" ${project.payment_type !== 'installment' ? 'selected' : ''}>通常</option>
                  <option value="installment" ${project.payment_type === 'installment' ? 'selected' : ''}>分割</option>
                </select>
              </div>
              <div><label>分割単価</label><input name="installment_amount" type="number" step="0.01" value="${this.ctx.escapeHtml(project.installment_amount ?? '')}" /></div>
              ...
              <div><label>開始時刻</label><input type="time" name="execution_time_start" value="${this.ctx.escapeHtml(this.kit.timeValue(project.execution_time_start))}" /></div>
              <div><label>終了時刻</label><input type="time" name="execution_time_end" value="${this.ctx.escapeHtml(this.kit.timeValue(project.execution_time_end))}" /></div>
              <div><label>拘束時間</label><input name="binding_time" type="number" step="0.25" value="${this.ctx.escapeHtml(project.binding_time ?? '')}" /></div>
              <div><label>休憩</label><input name="break_time" type="number" step="0.25" value="${this.ctx.escapeHtml(project.break_time ?? '')}" /></div>
            </div>
            ${
              id
                ? ''
