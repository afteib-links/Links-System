(() => {
  const LinksDailyReports = {
        ...
        .map((r, idx) => {
          ...
          const main = `
            <tr class="dr-main ${this.dayRowClass(date)}" data-idx="${idx}" data-work-date="${this.ctx.escapeHtml(date)}">
              ...
              <td class="dr-date-cell">${r.daily_report_id && r.source_file_id && this.canViewImportSource() ? `<button type="button" class="btn btn-small" data-ocr-records="${r.daily_report_id}" tabindex="-1">OCR記録</button>` : ''}${this.ctx.escapeHtml(this.formatDateWithWeekday(r.work_date))}${extraItems.length ? `<small class="dr-additional-mark" title="追加請求・支払項目あり">追加 ${extraItems.length}件</small>` : ''}${r.input_source_type && r.input_source_type !== 'manual' ? `<small class="dr-source-mark">${this.ctx.escapeHtml({excel:'Excel',email:'メール',fax:'FAX'}[r.input_source_type] || r.input_source_type)}${this.canViewImportSource() && r.source_file_id ? ` <a href="/api/daily-report-imports/files/${Number(r.source_file_id)}" target="_blank" rel="noopener">原本</a>` : ''}</small>` : ''}</td>
    ...
    bindGrid() {
      ...
      document.querySelectorAll('[data-ocr-records]').forEach(button=>button.onclick=()=>window.LinksPdfImports.showRecords(this.ctx,button.dataset.ocrRecords));
