(() => {
  ...
  const ui = {
    ...
    rowHtml(draft) {
      ...
      return `<article class="pdf-compare-row" data-row="${id}">
        ...
        <div class="pdf-values">${fields}<div class="pdf-extra-field pdf-observations">${this.observationsHtml(row)}</div><details class="pdf-row-details"><summary>対応先・原読取・理由 ${Object.keys(row.warnings).length ? '<span class="pdf-warning">要確認</span>' : ''}</summary><div class="pdf-target"><label>反映先<select data-target ${this.editable ? '' : 'disabled'}><option value="">${matches.length ? '反映先を選択' : '新規日報'}</option>${matches.map(r => `<option value="${r.daily_report_id}" ${Number(draft.target) === Number(r.daily_report_id) ? 'selected' : ''}>日報#${r.daily_report_id} / ${this.escape(r.start_time || '時刻未入力')} / ${this.escape(r.row_comment || '')}</option>`).join('')}${matches.length ? `<option value="new" ${draft.target === 'new' ? 'selected' : ''}>新しい作業行を追加する</option>` : ''}</select></label>
          ...
          <div class="pdf-raw-fields">${Object.keys(row.raw_data).map(field=>`<div><strong>${this.escape(labels[field] || observationLabels[field] || row.extra_data?.column_labels?.[field] || field)}</strong> 原読取: ${this.escape(row.raw_data[field] || '空欄')}${row.warnings[field]?`<span class="pdf-warning"> ${this.escape(row.warnings[field])}</span>`:''}${field!=='work_date'?`<label class="pdf-clear"><input type="checkbox" data-clear="${field}" ${draft.clears.has(field)?'checked':''} ${this.editable?'':'disabled'}> 明示消去</label>`:''}</div>`).join('')}</div>
    ...
    observationsHtml(row) {
      ...
      return Object.entries(row.observations||{}).filter(([k])=>k!=='date_annotation').map(([key,value])=>`<div><strong>${this.escape(observationLabels[key]||row.extra_data?.column_labels?.[key]||key)}</strong>: ${this.escape(state[value]||value||'空欄')} <small>原読取: ${this.escape(row.raw_data[key]||'空欄')}</small></div>`).join('')+
