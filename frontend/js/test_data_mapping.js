  ...
  window.LinksTestDataMapping = {
    ...
    initialize(host, sheets) {
      ...
      return sheets.map(s => {
        ...
        const type = saved?.type && host.meta.importFields[saved.type] ? saved.type : this.inferType(host, s);
        const used = new Set();
    ...
    render(host) {
      ...
      return `<section class="td-mapping"><div class="td-mapping-head"><div><h3>Excel列と取込項目の対応付け</h3>
        ...
        <div class="td-mapping-footer"><p role="status">${assigned.length}列を連携済み／${s.headers.length}列。未連携列は取り込みません。${host.importPending ? '保存時にこの連携を自動反映します。' : '設定へ反映済みです。'}</p>
