...
test('全機能のスクリプトを共通部品と依存順を保って読み込む', async () => {
  ...
  for (const key of ['companies', 'partners', 'base_projects', 'projects', 'price_sets', 'daily_reports', 'advances', 'invoices', 'payments', 'cash_management', 'analytics', 'master_settings', 'ui_builder', 'users']) {
    const loaded = [];
