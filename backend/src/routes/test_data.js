...
const { createOutputGenerationService } = require('../services/test_data/output_generation');
...
function createRouter(runQuery = query, env = process.env, suppliedGeneration = null, suppliedMonthly = null, suppliedSettlement = null, suppliedOutput = null) {
  ...
  router.get('/meta', (req, res) => res.json({ ok: true, defaults: model.defaults(), cases: model.CASES, presets: model.PRESETS,
    fields: Object.keys(imports.FIELDS), importFields: require('../services/test_data/fields').IMPORT_FIELDS, types: model.LABELS, generationAvailable: true, stage: 'outputs' }));
  ...
  router.get('/monthly-jobs', wrap(async (req,res)=>res.json({ok:true,jobs:await monthly.list()})));
  router.get('/monthly-jobs/:id', wrap(async (req,res)=>{
    const job=await monthly.get(req.params.id);
    if(!job)return res.status(404).json({ok:false,message:'月次生成ジョブが見つかりません'});
    return res.json({ok:true,job});
  }));
  ...
  router.get('/output-jobs', wrap(async (req,res)=>res.json({ok:true,jobs:await output.list()})));
  router.get('/output-jobs/:id', wrap(async (req,res)=>{
    const job=await output.get(req.params.id);
    if(!job)return res.status(404).json({ok:false,message:'帳票・入出金生成ジョブが見つかりません'});
    return res.json({ok:true,job});
  }));
  router.post('/settlement-jobs/:id/generate-outputs', wrap(async(req,res)=>{
    const job=await output.enqueue(req.params.id,req.session.user.user_id);
    return res.status(job.status==='completed'?200:202).json({ok:true,job});
  }));
