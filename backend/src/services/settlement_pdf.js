...
function renderHtml(document, lines) {
  ...
  return `<!doctype html><html lang="ja"><meta charset="utf-8"><style>
    @page { size:A4; margin:15mm; } body{font-family:"Noto Sans JP",sans-serif;color:#111;font-size:10pt} h1{text-align:center;font-size:22pt;letter-spacing:.2em;margin:0 0 18px} .meta{display:flex;justify-content:space-between;margin-bottom:14px}.recipient{font-size:14pt;margin:16px 0}.total{text-align:right;font-size:16pt;font-weight:bold;margin:14px 0} table{width:100%;border-collapse:collapse}th,td{border:1px solid #555;padding:6px}th{background:#eee}.num{text-align:right}.footer{margin-top:24px;border-top:1px solid #777;padding-top:8px;font-size:8pt;color:#555}</style>
    <body><h1>${titleFor(document.document_type)}</h1><div class="meta"><div>番号: ${escape(document.document_number)}<br>発行日: ${escape(document.issued_date)}</div><div>対象月: ${escape(document.target_year_month || '')}</div></div><div class="recipient">${escape(recipient)} 御中</div><div class="total">${document.settlement_type === 'invoice' ? 'ご請求額' : '振込予定額'}　${yen(document.total_amount)}</div><table><thead><tr><th>項目</th><th>数量</th><th>単価</th><th>金額</th></tr></thead><tbody>${rows}</tbody></table><div class="footer">本帳票は確定時点の明細・計算根拠を保存したPDFです。</div></body></html>`;
}
