...
const HELP_ROWS = [
  ...
  ['db_import','DB取込のヘルプ','編集用Excelを検証して、正常行だけDBへ登録・更新します。','出力後に画面で変更されたレコードは競合として除外します。'],
  ['db_export','DB出力のヘルプ','現行DBのマスター値を編集用Excelへ出力します。','修正したExcelはDB取込で検証します。Excelで消した行はDBから削除されません。'],
  ['master_data_export','マスターデータ出力のヘルプ','システムに同梱した基盤マスター初期値を取得します。','現行DBの値を確認する場合はDB出力を使用します。'],
  ['test_data','検証用データのヘルプ','検証用データを設計・確認します。','作成は検証専用環境・DBでだけ可能です。'],
  ['menu_access_settings','利用可能メニュー選択のヘルプ','役割ごとに利用できるメニューを設定します。','承認・取消などの追加制限は変更しません。'],
