...
async function main() {
  ...
  try {
    ...
    // Separate financial adoption from clock OCR, with a recalculation token.
    const empty={billing:'',payment:'',lineIds:{}};
    const fee={id:'normal',name:'通常',mode:'weekdays',weekdays:{weekday:true,sat:true,sun:true,holiday:true},matrix:{daily:{basic:{billing:10000,payment:8000},overtime:empty,night:empty,night_overtime:empty},hourly:{basic:empty,overtime:{billing:2000,payment:1000},night:empty,night_overtime:empty}}};
    await pool.query("INSERT INTO price_sets(price_set_no,price_set_name,company_id,project_id,apply_start_date,extra_data) VALUES (?,'採用匿名料金',?,?,'2026-09-01',?)",[`OCR-${project.insertId}`,company.insertId,project.insertId,JSON.stringify({fee_items:[fee]})]);
    let state=(await api(endpoint)).data;
    const existing=state.current_reports.find(r=>r.daily_report_id===current.daily_report_id);
    const [evidencePage]=await pool.query("INSERT INTO daily_report_pdf_pages(source_file_id,page_number,image_path,width,height,rectification) SELECT source_file_id,99,image_path,width,height,? FROM daily_report_pdf_pages WHERE pdf_page_id=?",[JSON.stringify({page_kind:'evidence'}),recognized.pages[0].pdf_page_id]);
    const adoption={import_row_id:first.daily_report_import_row_id,import_version:state.rows[0].version,target_daily_report_id:current.daily_report_id,expected_version:existing.version,fields:['work_date'],values:{work_date:'2026-09-01'},date_confirmed:true,reason:'帳票超過・経費を確認',quantity_overrides:{billing:{overtime_minutes:15,reason:'帳票照合'},payment:{overtime_minutes:30,reason:'契約照合'}},expense:{applies_to:'both',tax_category:'tax_inclusive',billing_amount:1500,payment_amount:1200,expected_version:null},reviewed_observations:{alcohol_check:'marked',confirmation_mark:'unknown'},evidence_pages:[evidencePage.insertId]};
    assert.equal((await api(endpoint+'/apply',{rows:[adoption]})).status,409,'計算未確認は反映不可');
    const preview=await api(endpoint+'/calculation-preview',adoption);assert.equal(preview.status,200,JSON.stringify(preview));
    assert.equal(preview.data.calculated.billing.overtime_minutes,15);assert.equal(preview.data.calculated.payment.overtime_minutes,30);
    adoption.preview_token=preview.data.preview_token;
    const adopted=await api(endpoint+'/apply',{rows:[adoption]});assert.equal(adopted.status,200,JSON.stringify(adopted));
    assert.equal((await api(endpoint+'/apply',{rows:[adoption]})).data.applied[0].skipped,true);
    let [expenses]=await pool.query('SELECT * FROM daily_additional_items WHERE request_key=?',[`ocr-expense-${current.daily_report_id}`]);
    assert.equal(expenses.length,1);assert.equal(Number(expenses[0].billing_amount),1500);assert.equal(expenses[0].tax_category,'tax_inclusive');
    const adoptedRecords=(await api(`/api/daily-report-imports/pdf/records/${current.daily_report_id}`)).data.records;
    assert.equal(adoptedRecords[0].reviewed_observations.alcohol_check,'marked');
    assert.equal(adoptedRecords[0].evidence[0].pdf_page_id,evidencePage.insertId);assert.equal(adoptedRecords[0].evidence[0].additional_item_id,expenses[0].additional_item_id);
    state=(await api(endpoint)).data;
    const reset={...adoption,import_version:state.rows[0].version,expected_version:state.current_reports.find(r=>r.daily_report_id===current.daily_report_id).version,quantity_overrides:{},reason:'自動計算へ戻す',expense:{...adoption.expense,expected_version:expenses[0].version,billing_amount:1600}};
    assert.equal((await api(endpoint+'/apply',{rows:[reset]})).status,409,'古い計算確認は使えない');
    reset.preview_token=(await api(endpoint+'/calculation-preview',reset)).data.preview_token;
