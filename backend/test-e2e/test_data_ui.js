...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.locator('.td-case-grid').evaluate(el => getComputedStyle(el).gridTemplateColumns.split(' ').length),3);
    assert.equal(await page.locator('#td-map-type').inputValue(),'companies');
    assert.equal(await page.evaluate(() => window.LinksTestDataMapping.inferType(window.LinksTestData,{name:'uploaddatas.xlsx / 稼働者一覧DB',headers:['担当','氏名','企業番号']})),'partners');
    ...
    await page.locator('#td-map-type').selectOption('partners');
    await page.locator('.td-mapping-list').first().evaluate(el => { el.scrollTop = 80; });
    await page.locator('#td-duplicate-policy').selectOption('last');
    assert.ok(await page.locator('.td-mapping-list').first().evaluate(el => el.scrollTop > 40));
    await page.locator('[data-map-target="name"]').click(); await page.locator('[data-map-column="0"]').click();
    await page.locator('#td-map-type').selectOption('companies');
    assert.ok((await page.locator('[data-map-target="workMode"]').innerText()).includes('第三列'));
    assert.ok((await page.locator('.td-map-target').first().boundingBox()).width > 250);
    ...
    await page.locator('#td-count-projects').fill('2');
    await page.locator('#td-preview').click();
    await page.locator('#td-sample-panel').waitFor();
    ...
    assert.equal(imported.catalog.partners[0].code,'P00001');
    assert.equal(imported.catalog.partners[1].name,'担当B');
    assert.equal(imported.importMappings[0].targets.length,2);
    ...
    console.log('PASS: preview/approval + bidirectional mapping, scroll retention, multi-master sheet, duplicate policy, persistence, reimport and mobile layout');
